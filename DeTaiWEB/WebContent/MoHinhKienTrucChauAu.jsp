...
<title><fmt:message key="EuropaST"/></title>
<style type="text/css">
...
<body onload="getPage(1,8,'EuropaSTProducts')">
  
    <fmt:setBundle basename="CreateUserGUI" scope="session"/> 
<%QuanLyMatHang qlmh=new QuanLyMatHang(); %>
<%ArrayList<MatHang> list=qlmh.getAllEuropaStructure(); 
...
<h2 style="border-bottom: 1px dotted black;"><fmt:message key="EuropaST"/></h2>
...
<div  id="khungChiTiet">
  <div id="tieuDeChiTiet" class="well"><h3><img src="image/castleIcon.png" width="40" height="40"><fmt:message key="EuropaST"/></h3></div>
   <div id="detailForEachType">
<%for(MatHang mh:list){ %>
  <div class="EuropaSTProducts">
...
<div id="phanTrangEuropaST">
  <ul class="pagination">
  
<%for(int i =1;i<=list.size()/8+1;i++){ %>
<li>
<a onclick="getPage(<%=i+8*(i-1)%>,<%=i*8%>,'EuropaSTProducts')"><%=i %></a>
</li>
<%} %>
  </ul>
</div>
...
<div class="fb-comments" data-href="http://localhost:8080/DeTaiWEB/MoHinhKienTrucChauAu.jsp" data-width="100%" data-numposts="5"></div>
</div>
</body>
</html>