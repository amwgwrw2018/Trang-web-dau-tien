...
<title><fmt:message key="Wonder"/></title>
<style type="text/css">
...
<body onload="getPage(1,8,'WOTWProducts')">
  
    <fmt:setBundle basename="CreateUserGUI" scope="session"/> 
<%QuanLyMatHang qlmh=new QuanLyMatHang(); %>
<%ArrayList<MatHang> list=qlmh.getProductWonderOfTheWorld(); 
...
<h2 style="border-bottom: 1px dotted black;"><fmt:message key="Wonder"/></h2>
...
<div  id="khungChiTiet">
  <div id="tieuDeChiTiet" class="well"><h3><img src="image/worldIcon.png" width="40" height="40"><fmt:message key="Wonder"/></h3></div>
   <div id="detailForEachType">
<%for(MatHang mh:list){ %>
  <div  class="WOTWProducts">
...
<div id="phanTrangKyQuan">
  <ul class="pagination">
  
<%for(int i =1;i<=list.size()/8+1;i++){ %>
<li>
<a onclick="getPage(<%=i+8*(i-1)%>,<%=i*8%>,'WOTWProducts')"><%=i %></a>
</li>
<%} %>
  </ul>
</div>
...
<div class="fb-comments" data-href="http://localhost:8080/DeTaiWEB/MoHinhKyQuanTheGioi.jsp" data-width="100%" data-numposts="5"></div>
</div>
</body>
</html>