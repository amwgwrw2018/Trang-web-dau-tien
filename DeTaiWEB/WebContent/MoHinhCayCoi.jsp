...
<body onload="getPage(1,8,'TreeProducts')">
<%!
...
<%ArrayList<MatHang> listTree=qlmh.getAllTree();
if(request.getAttribute("byPrice")!=null){
listTree.sort(new Comparator<MatHang>(){
...
<h2 style="border-bottom: 1px dotted black;"><fmt:message key="Tree"/></h2>
...
<div  id="khungChiTiet">
  <div id="tieuDeChiTiet" class="well"><h3><img src="image/treeIcon.png" width="40" height="40"><fmt:message key="Tree"/></h3></div>
  <div id="detailForEachType">
  <%for(MatHang mh:listTree){ %>
<%System.out.println(mh.getGia()); %>
...
<div id="phanTrangTree">
  <ul class="pagination">
  
<%for(int i =1;i<=listTree.size()/8+1;i++){ %>
<li>
<a onclick="getPage(<%=i+8*(i-1)%>,<%=i*8%>,'TreeProducts')"><%=i %></a>
</li>
<%} %>
  </ul>
</div>
...
<div class="fb-comments" data-href="http://localhost:8080/DeTaiWEB/MoHinhCayCoi.jsp" data-width="100%" data-numposts="5"></div>
</div>
