...
<title><fmt:message key="LivingHouse"/></title>
<style type="text/css">
...
<body onload="getPage(1,8,'LivingHouseProducts')">
  
    <fmt:setBundle basename="CreateUserGUI" scope="session"/> 
<%QuanLyMatHang qlmh=new QuanLyMatHang(); %>
<%ArrayList<MatHang> list=qlmh.getAllHouseForLiving(); 
...
<h2 style="border-bottom: 1px dotted black;"><fmt:message key="LivingHouse"/></h2>
...
<div  id="khungChiTiet">
  <div id="tieuDeChiTiet" class="well"><h3><img src="image/houseIcon.png" width="40" height="40"><fmt:message key="LivingHouse"/></h3></div>
   <div id="detailForEachType">
<%for(MatHang mh:list){ %>
  <div class="LivingHouseProducts" >
...
<div id="phanTrangLvHouse">
  <ul class="pagination">
  
<%for(int i =1;i<=list.size()/8+1;i++){ %>
<li>
<a onclick="getPage(<%=i+8*(i-1)%>,<%=i*8%>,'LivingHouseProducts')"><%=i %></a>
</li>
<%} %>
  </ul>
</div>
...
<div class="fb-comments" data-href="http://localhost:8080/DeTaiWEB/MoHinhNhaDanDung.jsp" data-width="100%" data-numposts="5"></div>
</div>
</body>
</html>