...
<title><fmt:message key="Cartoon"/></title>
<style type="text/css">
...
<body onload="getPage(1,8,'CartoonProducts')">
  
    <fmt:setBundle basename="CreateUserGUI" scope="session"/> 
<%QuanLyMatHang qlmh=new QuanLyMatHang(); %>
<%ArrayList<MatHang> list=qlmh.getAllCartoonCharacter(); 
...
<h2 style="border-bottom: 1px dotted black;"><fmt:message key="Cartoon"/></h2>
...
<div  id="khungChiTiet">
  <div id="tieuDeChiTiet" class="well"><h3><img src="image/Anime.png" width="40" height="40"><fmt:message key="Cartoon"/></h3></div>
   <div id="detailForEachType">
<%for(MatHang mh:list){ %>
  <div class="CartoonProducts">
...
<div id="phanTrangNVHH">
  <ul class="pagination">
  
<%for(int i =1;i<=list.size()/8+1;i++){ %>
<li>
<a onclick="getPage(<%=1+(8*(i-1))%>,<%=i*8%>,'CartoonProducts')"><%=i %></a>
</li>
<%} %>
  </ul>
</div>
...
<div class="fb-comments" data-href="http://localhost:8080/DeTaiWEB/MoHinhNhanVatHoatHinh.jsp" data-width="100%" data-numposts="5"></div>
</div>
</body>
</html>